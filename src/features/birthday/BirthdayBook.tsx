...
export function BirthdayBook({ config, onFinish }: Props) {
  ...
  const [paused, setPaused] = useState(false); const [imageFailed, setImageFailed] = useState(false);
  ...
  function finish() { window.localStorage.setItem('laoba:birthday-seen', '1'); onFinish(); }
  ...
  return <main className="birthday-book" onTouchStart={(e) => { touchStart.current = e.touches[0].clientX; }} onTouchEnd={endTouch}>
    <div className="birthday-tools"><span>家庭旅行册</span><div>{stage !== 'cover' && stage !== 'gift' && <button onClick={() => setPaused((value) => !value)} aria-label={paused ? '继续自动翻页' : '暂停自动翻页'}>{paused ? '继续' : '暂停'}</button>}{stage !== 'cover' && stage !== 'gift' && <button onClick={() => setStage('gift')} aria-label="跳过旅行照片">跳过</button>}</div></div>
    {stage === 'cover' && <section className="birthday-cover"><div className="travel-stamp">07 · 17<br />专属行程</div><div className="cover-copy"><p>今天的行程，你不用做攻略</p><h1>{config.coverTitle}</h1><p>{config.coverBody}</p><button className="birthday-cta" onClick={() => setStage('stops')}>出发</button><small>大约 1 分钟 · 随时可跳过</small></div></section>}
    {stage === 'stops' && <section className="trip-slide" key={index}><div className={`trip-photo ${stop.orientation}`}>{!imageFailed ? <img src={stop.image} alt={`${stop.place}旅行照片`} style={{ objectPosition: stop.objectPosition ?? 'center' }} onError={() => setImageFailed(true)} /> : <div className="photo-placeholder"><span>请替换旅行照片</span><strong>{stop.place}</strong></div>}</div><div className="trip-copy"><div><p>第 {String(index + 1).padStart(2, '0')} 站</p><h1>{stop.place}</h1><p className="memory-line">{stop.memory}</p></div><span className="trip-count">{String(index + 1).padStart(2, '0')} / {String(config.stops.length).padStart(2, '0')}</span></div><div className="trip-controls"><button disabled={index === 0} onClick={previous}>上一张</button><button className="birthday-cta" onClick={next}>下一张</button></div><div className="trip-progress"><i style={{ width: `${((index + 1) / config.stops.length) * 100}%` }} /></div></section>}
    ...
    {stage === 'gift' && <section className="birthday-gift"><p>十二站到这里</p><h1>爸，生日快乐。<br />下一站继续出发。</h1><p>这十二张照片，算是一份不太完整，但认真整理过的旅行记录。</p><p>新的一岁，祝你身体健康，吃得香、睡得好，想去的地方都有时间去。你和妈妈继续安排下一趟旅行，家里的房租、浇花和纪念日，就交给这个小工具帮忙记着。</p><div className="gift-ticket"><strong>家庭时光局</strong><span>每周、每月、每年提醒<br />重要事项可加入手机日历</span></div><button className="birthday-cta" onClick={finish}>打开我的提醒工具</button></section>}
  </main>;
}
