...
it('shows the birthday journey once and finishes at the reminder tool', () => {
  ...
  expect(screen.getByText('海南 · 三亚')).toBeInTheDocument();
  expect(screen.getByText('01 / 12')).toBeInTheDocument();
  const sanyaPhoto = screen.getByRole('img', { name: '海南 · 三亚旅行照片' });
  expect(sanyaPhoto).toHaveAttribute('src', expect.stringContaining('trips/01.jpg'));
  expect(sanyaPhoto.parentElement).toHaveClass('trip-photo', 'landscape');
